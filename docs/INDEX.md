# Docs Index

## Start Here
- `docs/00_SCOPE.md`
- `docs/01_POLYREPO_INTENT.md`
- `docs/02_ARCHITECTURE.md`
- `docs/65_DEV_GUIDE_DIRECTORY_MAP.md`
- `docs/67_REHEARSAL_COMMANDS.md`

## Current Entry Points (T097+)
- `docs/60_PIPELINE_TRAIN_CONTRACT.md` (pipeline v2)
- `docs/83_ENSEMBLE_POLICY.md` (ensemble)
- `docs/55_CLEARML_UI_CHECKLIST.md` (UI verification)
- `docs/69_CLEARML_TROUBLESHOOTING.md` (agent/git/commit pin)

## ClearML / UI Contracts
- `docs/03_CLEARML_UI_CONTRACT.md`
- `docs/81_CLEARML_TEMPLATE_POLICY.md`
- `docs/50_CLEARML_PROCESSED_DATASET_CONTRACT.md`
- `docs/51_CLEARML_PLOTS_SCALARS_DEBUGSAMPLES_CONTRACT.md`
- `docs/52_CLEARML_PIPELINE_CONTROLLER_CONTRACT.md`
- `docs/53_CLEARML_HYPERPARAMETERS_CONTRACT.md`
- `docs/60_PIPELINE_TRAIN_CONTRACT.md`
- `docs/61_CLEARML_HPARAMS_SECTIONS.md`
- `docs/62_CLEARML_PLOTS_REGRESSION.md`
- `docs/63_CLEARML_PIPELINES_VISIBILITY.md`
- `docs/64_INFER_BATCH_OPTIMIZE_CONTRACT.md`
- `docs/66_NAMING_TAGGING_POLICY.md`
- `docs/82_CLEARML_PROJECT_LAYOUT.md`

## Operations / Verification
- `docs/80_CLEARML_EXECUTION_MODES.md`
- `docs/10_OPERATION_MODES.md`
- `docs/15_VERIFICATION.md`
- `docs/16_OPERATIONS_RUNBOOK.md`
- `docs/40_CLEARML_TEST_PHASE_POLICY.md`
- `docs/42_REHEARSAL_SCENARIOS.md`
- `docs/84_REHEARSAL_GUIDE.md`
- `docs/55_CLEARML_UI_CHECKLIST.md`
- `docs/68_CLEARML_AGENT_TROUBLESHOOTING.md`
- `docs/69_CLEARML_TROUBLESHOOTING.md`
- `docs/70_CHATGPT_HANDOFF.md`

## Integration / Tracking
- `docs/06_ARTIFACTS_AND_VERSIONING.md`
- `docs/07_EVALUATION_PROTOCOL.md`
- `docs/83_ENSEMBLE_POLICY.md`
- `docs/13_PLATFORM_INTEGRATION.md`
- `docs/24_REPORTING.md`
