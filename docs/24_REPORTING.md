...
- `report_version`
- `grid_run_id`
- `summary` (recommended_model_id, primary_metric, best_score, models_tried, rationale)
- `dataset` (raw_dataset_id, processed_dataset_id, rows, feature_columns, target_column)
...
- `recommendation` (model_id, train_task_ref, thresholding/calibration/imbalance/uncertainty)
- `notes` (capabilities status)
- `next_actions` (operator guidance)

## report_links.json
`report_links.json` provides a quick navigation index:
- `pipeline`, `dataset_register`, `preprocess[]`, `train[]`, `leaderboard`, `infer`
