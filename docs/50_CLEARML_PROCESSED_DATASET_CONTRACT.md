...
- dataset name: `processed__{usecase_id}__{preprocess_variant}__{split_hash}__v{schema_version}`
- tags（例）: `usecase:{usecase_id}`, `process:preprocess`, `type:processed`, `schema:v{schema_version}`
...
- `splits.json`：split index/fold情報（preprocess 出力では `split.json` も併存）
...
- Train は split を Dataset の `splits.json` から復元し、再分割しない。
