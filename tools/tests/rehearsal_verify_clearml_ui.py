...
def _task_project(task: Any) -> str | None:
    for attr in ("project", "project_name"):
        value = getattr(task, attr, None)
        if value:
            return str(value)
    data = getattr(task, "data", None)
    project = getattr(data, "project", None) if data is not None else None
    if project:
        return str(project)
    return None
...
def _summarize_task(task: Any) -> dict[str, Any]:
    ...
    version_num = platform_adapter.normalize_clearml_version_num(script.get("version_num"))
...
def main() -> int:
    ...
    try:
        tasks = platform_adapter.list_clearml_tasks_by_tags([f"usecase:{args.usecase_id}"])
