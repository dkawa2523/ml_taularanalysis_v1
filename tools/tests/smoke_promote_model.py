#!/usr/bin/env python3
"""Smoke test for promote_model (local mode)."""
...
def _make_toy_csv(path: Path) -> None:
    ...
    df["target"] = 0.3 * df["num1"] - 0.1 * df["num2"] + (df["cat"] == "b").astype(float) + rng.normal(
        0, 0.1, size=n
    )
...
def main() -> int:
    ...
    smoke_root = repo / "work" / "_smoke_promote"
    tmp = smoke_root / "tmp"
    ...
    _run(
        [
            ...
            "run.clearml.enabled=false",
            f"run.output_dir={out_root}",
            f"promotion.source_leaderboard_dir={lb_dir}",
            "promotion.stage=staging",
            "promotion.note=smoke",
        ],
        cwd=repo,
    )
    pm_dir = out_root / "06_promote_model"
    _check_common(pm_dir)

    promotion = _load_json(pm_dir / "promotion.json")
    if "stage" not in promotion:
        raise AssertionError("promotion.json must contain stage")
    if "model_id" not in promotion:
        raise AssertionError("promotion.json must contain model_id")

    print("OK: promote_model")
    return 0
