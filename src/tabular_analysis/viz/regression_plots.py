...
def build_regression_metrics_table(
    ...
    *,
    metric_order: Sequence[str] = ("r2", "mse", "rmse", "mae"),
) -> Any | None:
    go = _plotly_go()
    if go is None:
        return None
    ...
    fig = go.Figure(
        data=[
            go.Table(
                header=dict(values=["metric", "value"], fill_color="#F2F2F2", align="left"),
    ...
    *,
    ...
    fig = go.Figure(
        go.Scatter(
            ...
            y=residuals,
            mode="markers",
            marker=dict(size=6, color="#F58518", opacity=0.7),
        )
    )
    fig.add_trace(
        go.Scatter(
            x=[float(np.nanmin(y_pred_arr)), float(np.nanmax(y_pred_arr))],
            y=[0.0, 0.0],
            mode="lines",
            line=dict(color="#333333", dash="dash"),
            name="zero",
