"""Lightweight data profile plots for ClearML PLOTS."""
...
def build_head_table(
    ...
    *,
    ...
    fig = go.Figure(
        data=[
            go.Table(
                header=dict(values=[str(col) for col in columns], fill_color="#E8EEF7", align="left"),
                cells=dict(values=values, fill_color="#F9FBFD", align="left"),
            )
        ]
    )
...
def build_missingness_bar(
    df: Any,
    *,
    columns: Sequence[str] | None = None,
    max_columns: int = 30,
    title: str = "Missing Rate by Column",
    output_dir: Path | None = None,
) -> Any | Path | None:
    go = _plotly_go()
    if go is None:
    ...
    fig = go.Figure(
        go.Bar(
            x=missing_rate.values,
            y=[str(col) for col in missing_rate.index],
            orientation="h",
            marker_color="#4C78A8",
        )
    )
...
def build_numeric_histograms(
    ...
    *,
    ...
    title_prefix: str = "Numeric Histogram",
    output_dir: Path | None = None,
) -> list[tuple[str, Any | Path | None]]:
    ...
    if go is None:
        return [
            (
                ...
                _fallback_image(
                    output_dir,
                    f"numeric_hist_{_slugify(col)}.png",
                    f"{title_prefix}: {col}",
                ),
            )
...
def build_categorical_topk_bars(
    df: Any,
    columns: Sequence[str],
    *,
    max_columns: int = 6,
    top_k: int = 10,
    sample_rows: int = 5000,
    title_prefix: str = "Top Categories",
    output_dir: Path | None = None,
) -> list[tuple[str, Any | Path | None]]:
    column_list = _limit_columns(list(columns), max_columns)
    if not column_list:
    ...
    if go is None:
        return [
            (
                str(col),
                _fallback_image(
                    output_dir, f"categorical_topk_{_slugify(col)}.png", f"{title_prefix}: {col}"
                ),
            )
            for col in column_list
        ]
...
def build_target_distribution(
    ...
    *,
    ...
    if is_numeric_dtype(series) and not is_bool_dtype(series):
        results = build_numeric_histograms(
            df,
            [target_column],
            max_columns=1,
            bins=bins,
        ...
        return results[0][1] if results else _fallback_image(
            output_dir, "target_distribution.png", title
        )
    results = build_categorical_topk_bars(
        df,
        [target_column],
        max_columns=1,
        top_k=top_k,
...
def build_profile_comparison_table(
    ...
    *,
    title: str = "Raw vs Processed Summary",
    output_dir: Path | None = None,
) -> Any | Path | None:
    go = _plotly_go()
    if go is None:
    ...
    fig = go.Figure(
        data=[
            go.Table(
                header=dict(
                    values=["metric", "raw", "processed"], fill_color="#E8EEF7", align="left"
                ),
                cells=dict(
                    values=[metric_labels, raw_values, processed_values],
                    fill_color="#F9FBFD",
                    align="left",
                ),
            )
...
def build_missing_rate_comparison_bar(
    raw_rate: float,
    processed_rate: float,
    *,
    title: str = "Missing Rate (raw vs processed)",
    output_dir: Path | None = None,
) -> Any | Path | None:
    go = _plotly_go()
    if go is None:
