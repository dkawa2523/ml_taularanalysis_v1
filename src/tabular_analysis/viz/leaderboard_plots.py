"""Plotly visuals for leaderboard."""
...
def build_leaderboard_table(
    ...
    *,
    ...
    row_colors = [
        "#FFF4CC" if (row.get("rank") == 1) else "#FFFFFF" for row in view_rows
    ]
    ...
    fig = go.Figure(
        data=[
            go.Table(
                ...
                cells=dict(values=cell_values, fill_color=fill_colors, align="left"),
            )
        ]
    )
...
def build_top_k_bar(
    ...
    *,
    ...
    fig.update_layout(
        title=title or f"Top-K {score_label}",
...
def build_pareto_scatter(
    ...
    *,
    ...
    fig.update_layout(
        title=title or f"Pareto: {x_metric} vs {y_metric}",
