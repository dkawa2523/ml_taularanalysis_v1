"""Drift profiling and PSI reporting."""
...
def _infer_feature_types(df, feature_columns: Iterable[str]) -> tuple[list[str], list[str]]:
    try:
...
def _normalize_metrics(metrics: Sequence[str] | None) -> list[str]:
    if not metrics:
        return ["psi"]
    normalized: list[str] = []
    seen: set[str] = set()
    ...
    *,
    ...
    for col in numeric_list:
        ...
        numeric_profile[str(col)] = {
            ...
            "hist": {"bin_edges": edges, "counts": counts},
        }
    ...
    for col in categorical_list:
        ...
        categorical_profile[str(col)] = {
            "count": total,
            "missing_count": missing_count,
            "top": top_entries,
            "other": {"count": other_count, "rate": other_rate},
        }
...
def build_drift_report(
    train_profile: Mapping[str, Any],
    infer_df,
    *,
    psi_warn_threshold: float = 0.2,
    psi_fail_threshold: float | None = 0.4,
    strict: bool = False,
    metrics: Sequence[str] | None = None,
    train_profile_path: str | None = None,
) -> dict[str, Any]:
    try:
        import pandas as pd  # type: ignore
    except Exception as exc:
    ...
    metrics_list = _normalize_metrics(metrics)
    ...
    report: dict[str, Any] = {
        ...
        "thresholds": {
            ...
            "fail": float(fail_threshold) if strict and fail_threshold is not None else None,
        },
        "numeric": {},
        "categorical": {},
    }
    ...
    for col, info in train_numeric.items():
        ...
        if not edges or len(edges) < 2 or len(train_counts) != len(edges) - 1:
            numeric_report[str(col)] = {
                "psi": None,
                "status": "n/a",
                "reason": "invalid_histogram",
            }
            continue
        train_count = _to_int((info or {}).get("count"))
        train_missing = _to_int((info or {}).get("missing_count"))
        ...
        entry = {
            "psi": psi_value,
            "status": status,
            "missing_rate_train": _missing_rate(train_count, train_missing),
            "missing_rate_infer": _missing_rate(int(non_missing.shape[0]), missing_count),
            "expected": {
                "bin_edges": [float(v) for v in edges],
                "counts": [int(v) for v in train_counts],
            },
            "actual": {
                "counts": [int(v) for v in actual_counts],
            },
        }
        if compute_ks:
            entry["ks"] = ks_value
    ...
    for col, info in train_categorical.items():
        ...
        train_top_counts = {str(entry.get("value")): _to_int(entry.get("count")) for entry in top_entries}
        train_other = _to_int((info or {}).get("other", {}).get("count"))
        ...
        categorical_report[str(col)] = {
            ...
            "status": status,
            "missing_rate_train": _missing_rate(train_count, train_missing),
            "missing_rate_infer": _missing_rate(total, missing_count),
            "top_values": top_values,
            "expected": {
                "counts": [int(v) for v in expected],
                "labels": [*top_values, "other"],
            },
            "actual": {
                "counts": [int(v) for v in actual],
                "labels": [*top_values, "other"],
            },
        }
    ...
    report["summary"] = {
        "features_total": len(numeric_report) + len(categorical_report),
        "psi_max": psi_max,
        "psi_mean": psi_mean,
        "ks_max": ks_max,
        "ks_mean": ks_mean,
        "warn_threshold": float(warn_threshold),
        "fail_threshold": float(fail_threshold) if strict and fail_threshold is not None else None,
        "warn_count": len(warned_features),
...
def _collect_top_features(report: Mapping[str, Any], *, limit: int = 10) -> list[dict[str, Any]]:
    ...
    for kind in ("numeric", "categorical"):
        ...
        for name, entry in section.items():
            ...
            ks_value = entry.get("ks")
            if ks_value is not None:
                item["ks"] = float(ks_value)
            candidates.append(item)
    candidates.sort(key=lambda item: item["psi"], reverse=True)
...
def render_drift_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- psi_warn_threshold: {summary.get('warn_threshold')}",
        f"- psi_fail_threshold: {summary.get('fail_threshold')}",
        f"- psi_max: {summary.get('psi_max')}",
        f"- psi_mean: {summary.get('psi_mean')}",
        f"- ks_max: {summary.get('ks_max')}",
        f"- ks_mean: {summary.get('ks_mean')}",
    ...
    top_features = _collect_top_features(report, limit=10)
    if top_features:
        ...
        for item in top_features:
            ...
            lines.append(
                f"- {item['feature']} ({item['kind']}): psi={item['psi']:.4f}{ks_text} "
                f"[{item.get('status')}]"
            )
