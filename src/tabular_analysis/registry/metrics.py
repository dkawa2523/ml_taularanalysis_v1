...
def _extract_positive_proba(y_proba: Any) -> Any:
    ...
    if arr.ndim == 2 and arr.shape[1] >= 2:
        return arr[:, 1]
    raise ValueError("probability array must have at least 2 columns for roc_auc.")
    ...
    if task == "classification":
        ...
        if key in ("fbeta", "f_beta", "fbeta_score"):
            ...
            try:
                ...
                beta = 1.0
            return lambda y_true, y_pred, y_proba=None: float(
                fbeta_score(y_true, y_pred, beta=beta, average=avg)
            )
        ...
        if key in ("roc_auc", "auc"):
            ...
            def _roc_auc(y_true, y_pred=None, y_proba=None):
                ...
                return float(roc_auc_score(y_true, _extract_positive_proba(y_proba)))
        ...
        if key in ("pr_auc", "average_precision", "average_precision_score"):
            ...
            def _pr_auc(y_true, y_pred=None, y_proba=None):
                ...
                return float(average_precision_score(y_true, _extract_positive_proba(y_proba)))
