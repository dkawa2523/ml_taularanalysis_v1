"""Categorical feature encoders for high-cardinality support."""
...
class TargetMeanEncoder:
    ...
    def fit(self, X, y: Any | None = None) -> "TargetMeanEncoder":
        ...
        self.columns_ = [str(col) for col in df.columns]
        self.mapping_, self.global_mean_ = _compute_target_mean_mapping(
            df,
            y_arr,
...
class TabularPreprocessor:
    def __init__(
        ...
        *,
        ...
        numeric_pipeline: Pipeline | None,
        categorical_imputer: SimpleImputer | None,
        categorical_encoder: Any | None,
    ) -> None:
    ...
    def transform_with_oof(
        ...
        *,
        ...
        oof_encoded = target_mean_oof_encode(
            cat_df.iloc[train_idx_array],
            y_arr[train_idx_array],
            folds=folds,
            smoothing=self.categorical_encoder.smoothing,
            seed=seed,
...
def _compute_target_mean_mapping(
    df: pd.DataFrame, y: np.ndarray, *, smoothing: float
    ...
    for col in df.columns:
        ...
        smoothed = (stats["count"] * stats["mean"] + smoothing_value * global_mean) / (
            stats["count"] + smoothing_value
        )
...
def _apply_target_mean(
    df: pd.DataFrame, mapping: Mapping[str, Mapping[Any, float]], global_mean: float
) -> np.ndarray:
...
def target_mean_oof_encode(
    ...
    *,
    ...
    for train_idx, val_idx in split_iter:
        mapping, global_mean = _compute_target_mean_mapping(
            df.iloc[train_idx], y_arr[train_idx], smoothing=float(smoothing or 0.0)
        )
        encoded[val_idx] = _apply_target_mean(df.iloc[val_idx], mapping, global_mean)
    return encoded


def encode_target_for_mean(
    train_values: Iterable[Any],
    ...
    *,
    ...
    y_train = encoder.fit_transform(list(train_values))
    ...
    return (
        y_all.astype(float),
        {"task_type": task_type, "n_classes": n_classes, "class_labels": [str(v) for v in encoder.classes_]},
    )


def build_tabular_preprocessor(
    preprocess_variant: Mapping[str, Any],
    *,
    numeric_features: Sequence[str],
    categorical_features: Sequence[str],
    numeric_impute: str,
    ...
    target_mean_smoothing: float | None,
    unique_counts: Mapping[str, int] | None,
) -> tuple[TabularPreprocessor, dict[str, str], str]:
    ...
    if encoding == "auto":
        ...
        if onehot_cols:
            encoders.append(
                ("onehot", OneHotEncoder(handle_unknown=str(handle_unknown), sparse_output=False), onehot_cols)
            )
        if hash_cols:
...
def build_categorical_encoding_report(
    categorical_features: Sequence[str],
    *,
    encoding: str,
    encoding_by_column: Mapping[str, str],
