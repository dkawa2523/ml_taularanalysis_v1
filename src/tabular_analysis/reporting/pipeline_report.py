"""Pipeline report generation for markdown/json/links outputs."""
...
def _normalize_top_model_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    normalized["best_score"] = _coerce_float(row.get("best_score"))
    normalized["primary_metric_ci_low"] = _coerce_float(row.get("primary_metric_ci_low"))
    normalized["primary_metric_ci_mid"] = _coerce_float(row.get("primary_metric_ci_mid"))
    normalized["primary_metric_ci_high"] = _coerce_float(row.get("primary_metric_ci_high"))
    for key in (
        "primary_metric",
        "task_type",
        "model_id",
...
def _summarize_data_quality(data_quality: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "scan_sampled": data_quality.get("scan_sampled"),
...
def _summarize_data_quality_from_out(summary: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "rows_scanned": summary.get("rows"),
        "duplicates_count": summary.get("duplicates_count"),
        "missing_columns": summary.get("missing_columns"),
        "leak_suspect_count": summary.get("leak_suspect_count") or summary.get("leak_suspects"),
    }


def _collect_report_payload(
    pipeline_run: Mapping[str, Any], *, cfg: Any | None, max_models: int
) -> dict[str, Any]:
    ...
    data_quality_payload = _safe_load_json(
        resolver.resolve(dataset_register_ref, "data_quality.json")
    ) or {}
    ...
    target_column = preprocess_schema.get("target_column") or dataset_out.get("raw_schema", {}).get(
        "target_column"
    )
    ...
    recommended_model_id = _normalize_str(
        recommended_detail.get("model_id")
        or recommendation.get("recommended_model_id")
        or leaderboard_out.get("recommended_model_id")
    )
    recommended_metric = _normalize_str(
        recommended_detail.get("primary_metric")
        or recommendation.get("recommended_primary_metric")
    ...
    recommended_train_ref = _normalize_str(
        recommended_detail.get("train_task_ref")
        or recommendation.get("recommended_train_task_ref")
        or leaderboard_out.get("recommended_train_task_ref")
        or leaderboard_out.get("recommended_train_task_id")
    )

    thresholding = (
        recommended_detail.get("thresholding")
        if isinstance(recommended_detail.get("thresholding"), Mapping)
        else None
    )
    ...
    if not comparability:
        comparability = {
            "processed_dataset_id": _normalize_str(
                preprocess_ref.get("processed_dataset_id") if preprocess_ref else None
            ),
    ...
    if isinstance(decision_summary.get("top_models"), list):
        top_models = [
            _normalize_top_model_row(item)
            for item in decision_summary.get("top_models") or []
    ...
    return {
        ...
        "summary": {
            ...
            "train_task_ref": recommended_train_ref,
            "planned_jobs": planned_jobs,
            "executed_jobs": executed_jobs,
            "skipped_due_to_policy": skipped_jobs,
            "plan_only": plan_only,
            "recommendation_rationale": rationale,
        },
        "dataset": {
            "raw_dataset_id": dataset_out.get("raw_dataset_id"),
            "processed_dataset_id": _normalize_str(
                preprocess_ref.get("processed_dataset_id") if preprocess_ref else None
            )
            or comparability.get("processed_dataset_id"),
            "rows": rows,
        ...
        "split": {
            ...
            "test_size": split_payload.get("test_size"),
        ...
        "top_models": top_models[:max_models],
        "recommendation": {
            ...
            "best_score": recommended_score,
            "direction": direction,
            "primary_metric_ci": recommended_detail.get("primary_metric_ci"),
            "task_type": task_type,
            "n_classes": recommended_detail.get("n_classes"),
            "threshold": recommended_threshold,
            "thresholding": notes.get("thresholding"),
            "calibration": notes.get("calibration"),
            "imbalance": notes.get("imbalance"),
            "uncertainty": notes.get("uncertainty"),
        },
        "notes": notes,
...
def _render_report_markdown(payload: Mapping[str, Any], *, max_models: int) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "## Conclusion",
        f"- grid_run_id: {_format_value(payload.get('grid_run_id'))}",
        f"- recommended_model_id: {_format_value(summary.get('recommended_model_id'))}",
        f"- primary_metric: {_format_value(summary.get('primary_metric'))}",
        f"- best_score: {_format_value(summary.get('best_score'))}",
    ...
    lines.extend(
        [
            "",
            "## Data Overview",
            f"- raw_dataset_id: {_format_value(dataset.get('raw_dataset_id'))}",
            f"- processed_dataset_id: {_format_value(dataset.get('processed_dataset_id'))}",
            f"- rows: {_format_value(dataset.get('rows'))}",
            f"- feature_columns: {_format_value(dataset.get('feature_columns'))}",
            f"- target_column: {_format_value(dataset.get('target_column'))}",
        ]
    )
    ...
    lines.extend(
        [
            "",
            "## Comparability",
            f"- require_comparable: {_format_value(comparability.get('require_comparable'))}",
            f"- processed_dataset_id: {_format_value(comparability.get('processed_dataset_id'))}",
            f"- split_hash: {_format_value(comparability.get('split_hash'))}",
            f"- recipe_hash: {_format_value(comparability.get('recipe_hash'))}",
            f"- primary_metric: {_format_value(comparability.get('primary_metric'))}",
            f"- direction: {_format_value(comparability.get('direction'))}",
            f"- task_type: {_format_value(comparability.get('task_type'))}",
    ...
    lines.extend(
        [
            "",
            "## Split / Recipe / Hashes",
            f"- preprocess_variant: {_format_value(split.get('preprocess_variant'))}",
            f"- split.strategy: {_format_value(split.get('strategy'))}",
            f"- split.test_size: {_format_value(split.get('test_size'))}",
            f"- split.seed: {_format_value(split.get('seed'))}",
            f"- split_hash: {_format_value(split.get('split_hash'))}",
            f"- recipe_hash: {_format_value(split.get('recipe_hash'))}",
        ]
    )
    ...
    lines.extend(
        [
            "",
            "## Recommendation",
            f"- model_id: {_format_value(recommendation.get('model_id'))}",
            f"- primary_metric: {_format_value(recommendation.get('primary_metric'))}",
            f"- best_score: {_format_value(recommendation.get('best_score'))}",
        ]
    )
    ...
    if isinstance(thresholding, Mapping) and thresholding.get("best_threshold") is not None:
        lines.append(
            "- thresholding: enabled metric={metric} best_threshold={thr} score={score}".format(
                ...
                thr=_format_value(thresholding.get("best_threshold")),
                score=_format_value(thresholding.get("score")),
            )
        )
    else:
    ...
    if isinstance(uncertainty, Mapping) and uncertainty.get("enabled"):
        lines.append(
            "- uncertainty: enabled method={method} alpha={alpha} q={q}".format(
                ...
                alpha=_format_value(uncertainty.get("alpha")),
                q=_format_value(uncertainty.get("q")),
            )
        )
    else:
...
def build_pipeline_report_links(
    ...
    *,
    ...
    pipeline_entry = _build_link_entry(
    ...
    if isinstance(dataset_ref, Mapping):
        dataset_entry = _build_link_entry(
            cfg, _normalize_str(dataset_ref.get("run_dir")), _normalize_str(dataset_ref.get("task_id"))
        )
    ...
    for ref in pipeline_run.get("train_refs") or []:
        ...
        entry = _build_link_entry(
            cfg,
            _normalize_str(ref.get("run_dir")),
            _normalize_str(ref.get("task_id") or ref.get("train_task_id")),
            extra={
    ...
    if isinstance(leaderboard_ref, Mapping):
        leaderboard_entry = _build_link_entry(
            cfg,
            _normalize_str(leaderboard_ref.get("run_dir")),
            _normalize_str(leaderboard_ref.get("task_id")),
        )
    ...
    if isinstance(infer_ref, Mapping):
        infer_entry = _build_link_entry(
            cfg, _normalize_str(infer_ref.get("run_dir")), _normalize_str(infer_ref.get("task_id"))
        )
