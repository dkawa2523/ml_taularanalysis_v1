...
_DEFAULT_NAME_PATTERNS = {
    "leak_suspect": ("target", "label", "outcome", "response"),
    "id_suspect": ("id", "uuid", "guid"),
}
...
def _threshold_float(
    thresholds: Mapping[str, Any] | None, key: str, default: float
) -> float:
    if not thresholds:
...
def _normalize_patterns(
    name_patterns: Mapping[str, Any] | None, target_column: str | None
) -> dict[str, list[str]]:
    patterns: dict[str, list[str]] = {
        key: [str(item).lower() for item in values if str(item).strip()]
        for key, values in _DEFAULT_NAME_PATTERNS.items()
    }
...
def _sample_type_buckets(
    series, *, max_samples: int, np: Any | None, pd: Any | None
) -> dict[str, int]:
    try:
...
def _is_integer_like(series) -> bool:
    try:
        from pandas.api.types import is_bool_dtype, is_integer_dtype  # type: ignore
    except Exception:
        dtype_text = str(getattr(series, "dtype", "")).lower()
...
def compute_data_quality(
    ...
    task_type: str | None,
    id_columns: Iterable[Any] | None = None,
    *,
    max_rows_scan: int = 50000,
    thresholds: Mapping[str, Any] | None = None,
    name_patterns: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    high_card_ratio = _threshold_float(
        thresholds, "high_cardinality_ratio_warn", _HIGH_CARDINALITY_THRESHOLD
    )
    high_card_min_unique = _threshold_int(
        thresholds, "high_cardinality_min_unique", _HIGH_CARDINALITY_MIN_UNIQUE
    )
    ...
    near_match_threshold = _threshold_float(
        thresholds, "leak_near_match_warn", _NEAR_MATCH_THRESHOLD
    )
    corr_threshold = _threshold_float(
        thresholds, "leak_correlation_warn", _CORRELATION_THRESHOLD
    )
    mapping_threshold = _threshold_float(
        thresholds, "leak_mapping_accuracy_warn", _MAPPING_ACCURACY_THRESHOLD
    )
    ...
    try:
        for col in df_scan.columns:
                    ...
                    id_like_columns.append(
                        {
                            ...
                            "declared_id": col_name in id_columns_set,
                        }
                    )
            ...
            if rows_scanned > 0 and "object" in dtype_text:
                type_buckets = _sample_type_buckets(
                    series, max_samples=_MAX_TYPE_SAMPLE, np=np, pd=pd
                )
                if len(type_buckets) > 1:
                    mixed_type_columns.append(col_name)
                    mixed_type_details.append(
                        {"column": col_name, "types": dict(type_buckets)}
                    )

            if leak_name_patterns:
                matches = _pattern_matches(col_name, leak_name_patterns)
                if matches:
                    name_suspects.append(
                        {"column": col_name, "kind": "leak_suspect", "patterns": matches}
                    )
            if id_name_patterns:
    ...
    leak_summary: dict[str, Any] = {
        ...
        "mapping_suspects": [],
        "thresholds": {
            "near_match": near_match_threshold,
            "correlation": corr_threshold,
            "mapping_accuracy": mapping_threshold,
        },
    }
    ...
    if target_column and target_column in getattr(df_scan, "columns", []):
        ...
        feature_columns = [
            col
            for col in df_scan.columns
            if str(col) != target_column and str(col) not in id_columns_list
        ]
        for col in feature_columns:
            ...
            if _series_equals(series, target_series):
                ...
                _add_suspect(
                    column=str(col), reason="exact_match", severity="error", score=1.0
                )
                continue
            match_rate = _match_rate(series, target_series)
            if match_rate >= near_match_threshold:
                leak_summary["near_match"].append(
                    {"column": str(col), "match_rate": float(match_rate)}
                )
                _add_suspect(
                    column=str(col),
                    reason="near_match",
        ...
        if task_type == "regression":
            ...
            if pd is not None and rows_scanned:
                for col in feature_columns:
                    ...
                    counts = (
                        pd.DataFrame({"col": col_values, "target": target_values})
                        .groupby(["col", "target"], dropna=False)
                        .size()
                    )
                    if counts.empty:
                        continue
                    ...
                    if deterministic or accuracy >= mapping_threshold:
                        leak_summary["mapping_suspects"].append(
                            {
                                "column": str(col),
                                "accuracy": accuracy,
                                "unique_values": int(target_per_col.shape[0]),
    ...
    payload = {
        ...
        "scan_sampled": sampled,
        "target_column": target_column,
        "task_type": task_type,
        "id_columns": id_columns_list,
        "dtype_counts": dtype_counts,
        "missing_total": missing_total,
        "missing_rate_total": missing_rate_total,
        ...
        "mixed_type_details": mixed_type_details,
        "high_cardinality_columns": high_cardinality_columns,
        "high_cardinality_details": high_cardinality_details,
        "high_cardinality_threshold": high_card_ratio,
        "id_like_columns": id_like_columns,
        "name_suspects": name_suspects,
        "quality_thresholds": {
            "high_cardinality_ratio_warn": high_card_ratio,
            "high_cardinality_min_unique": high_card_min_unique,
            "id_like_ratio_warn": id_like_ratio,
            "id_like_min_unique": id_like_min_unique,
            "leak_near_match_warn": near_match_threshold,
            "leak_correlation_warn": corr_threshold,
            "leak_mapping_accuracy_warn": mapping_threshold,
            "mixed_type_columns_warn": thresholds.get("mixed_type_columns_warn"),
            "mixed_type_columns_fail": thresholds.get("mixed_type_columns_fail"),
        },
...
def summarize_data_quality(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    leak_columns = [
        str(item.get("column"))
        for item in leak_suspects
        if item and item.get("column") is not None
    ]
    leak_error = any(
        (item or {}).get("severity") == "error" for item in leak_suspects if item is not None
    )
    summary = {
        ...
        "duplicates_count": payload.get("duplicates_count"),
        "high_cardinality_count": len(payload.get("high_cardinality_columns") or []),
        "id_like_count": len(payload.get("id_like_columns") or []),
        "mixed_type_count": len(payload.get("mixed_type_columns") or []),
        "name_suspect_count": len(payload.get("name_suspects") or []),
        "leak_suspect_count": len(leak_suspects),
        "leak_suspects": leak_columns,
        "leak_error": leak_error,
        "quality_status": payload.get("quality_status"),
        "quality_issue_count": payload.get("quality_issue_count"),
    }
    return summary
...
def render_data_quality_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- sampled: {bool(sampled)}",
            f"- missing_columns: {payload.get('missing_columns')}",
            f"- missing_rate_total: {payload.get('missing_rate_total'):.6f}"
            if isinstance(payload.get("missing_rate_total"), float)
            else f"- missing_rate_total: {payload.get('missing_rate_total')}",
    ...
    lines.append(
        "- constant_columns: " + (", ".join(constant_cols) if constant_cols else "n/a")
    )
    ...
    lines.append(
        "- high_cardinality_columns: "
        + (", ".join(high_card_cols) if high_card_cols else "n/a")
    )
