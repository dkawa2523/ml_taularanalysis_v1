"""Data quality gate execution and reporting."""
...
from ..platform_adapter import TaskContext, update_task_properties, upload_artifact
...
def _merge_patterns(raw: Any) -> dict[str, list[str]]:
    merged: dict[str, list[str]] = {
        key: list(values) for key, values in _DEFAULT_NAME_PATTERNS.items()
    }
    payload = _to_container(raw)
    if isinstance(payload, Mapping):
...
def resolve_quality_settings(cfg: Any) -> dict[str, Any]:
    ...
    legacy_fail_duplicates = _cfg_value(cfg, "data.quality.fail_on_duplicates", None)
    legacy_fail_leak = _cfg_value(cfg, "data.quality.fail_on_leak", None)
    if raw_mode is None:
        if legacy_fail_duplicates or legacy_fail_leak:
            mode = "fail"
        else:
            mode = "warn"
    else:
...
def evaluate_quality(payload: dict[str, Any], thresholds: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if missing_rate is not None:
        if missing_fail is not None and missing_rate >= missing_fail:
            _add_issue(
                ...
                count=1,
                detail={
                    "missing_rate_total": missing_rate,
                    "fail_threshold": missing_fail,
                },
            )
    ...
    if duplicates_rate is not None:
        if dup_fail is not None and duplicates_rate >= dup_fail:
            _add_issue(
                ...
                severity="fail",
                count=duplicates_count or 1,
                detail={
                    "duplicates_rate": duplicates_rate,
                    "fail_threshold": dup_fail,
                },
            )
    ...
    if const_count > 0:
        ...
        _add_issue(
            ...
            severity=severity,
            count=const_count,
            detail={"columns": _limit_list(constant_cols)},
        )
    ...
    if mixed_count > 0:
        ...
        _add_issue(
            issues,
            issue_type="mixed_type_columns",
            severity=severity,
            count=mixed_count,
            detail={"columns": _limit_list(mixed_type_details)},
        )
    ...
    if high_card_count > 0:
        ...
        _add_issue(
            ...
            severity=severity,
            count=high_card_count,
            detail={"columns": _limit_list(high_card_details)},
        )
    ...
    if id_like_count > 0:
        ...
        _add_issue(
            ...
            severity=severity,
            count=id_like_count,
            detail={"columns": _limit_list(id_like_cols)},
        )
    ...
    if name_count > 0:
        _add_issue(
            issues,
            issue_type="name_suspect",
            severity="warn",
            count=name_count,
            detail={"columns": _limit_list(name_suspects)},
        )
    ...
    if leak_errors:
        _add_issue(
            ...
            issue_type="leak",
            severity="fail",
            count=len(leak_errors),
            detail={"columns": _limit_list(leak_errors)},
        )
    if leak_warnings:
        _add_issue(
            issues,
            issue_type="leak",
            severity="warn",
            count=len(leak_warnings),
            detail={"columns": _limit_list(leak_warnings)},
        )
...
def _build_disabled_payload(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "task_type": task_type,
        "id_columns": [str(c) for c in (id_columns or [])],
        "dtype_counts": {},
        "missing_total": None,
        "missing_rate_total": None,
        ...
        "high_cardinality_threshold": thresholds.get("high_cardinality_ratio_warn"),
        "id_like_columns": [],
        "name_suspects": [],
        "quality_thresholds": dict(thresholds),
        "leak_suspects": [],
        "leak_summary": {"available": False, "note": "data_quality_disabled_or_unavailable"},
    }
...
def _write_quality_artifacts(
    payload: Mapping[str, Any], output_dir: Path, prefix: str
) -> tuple[Path, Path]:
...
def run_data_quality_gate(
    *,
    ...
    artifact_prefix: str = "data_quality",
) -> dict[str, Any]:
    ...
    if df is None or not settings["enabled"]:
        payload = _build_disabled_payload(
            schema=schema,
            target_column=target_column,
            task_type=task_type,
        ...
        payload = compute_data_quality(
            df,
            target_column=target_column,
            task_type=task_type,
            id_columns=id_columns,
            max_rows_scan=settings["max_rows_scan"],
            thresholds=thresholds,
            name_patterns=settings["name_patterns"],
        )
    ...
    update_task_properties(
        ctx, {"quality_status": str(status), "quality_issue_count": str(issue_count)}
    )
    ...
    gate = {
        "mode": settings["mode"],
        "status": status,
        "should_fail": settings["mode"] == "fail" and status == "fail",
        "issue_count": issue_count,
        "fail_reasons": _format_fail_reasons(issues),
    }
    return {
        "payload": payload,
        "summary": summarize_data_quality(payload),
        "gate": gate,
        "paths": {"json": json_path, "md": md_path},
    }


def raise_on_quality_fail(
    *,
    cfg: Any,
    ctx: TaskContext,
    ...
    emit_alert(
        ...
        message,
        {
            ...
            "data_quality_path": str(json_path),
        },
    )
